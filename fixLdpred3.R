allNames=read.table("allNames3",stringsAsFactors=F)
#print(allNames)
...
for(fName in allNames[,1]){
	...
	for(i in 1:3){
		system(paste("zcat",paste0(fName,".",i,".final.set.ldpred.gz"),"| grep chrom > readFrom3"))
		print(i)
		fixFile=read.table("readFrom3",stringsAsFactors=F,header=F)
		blankFix=c()
		for(j in 1:22){
			gFile=allNormal[[j]]
