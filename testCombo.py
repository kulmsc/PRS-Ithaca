...
for q in allFilesReal:
	if q.split('.')[1].split('-')[0]==method: #change to either 3 or 1
		allFiles.append(q)
if len(allFiles)==0:
	sys.exit()
...
for data in allFiles:
	splitData=data.split('.')[1].split('-')[1]
	print(splitData)
...
for i in range(22):
	tempList=[]
	for poss in os.listdir("/home/cbsumezey/sdk2004/chr"+str(i+1)):
		if poss[0:5]!="field":
...
for scoreType,scoreLabel in zip(master,masterLabel):
	...
	gzWrite(scoreLabel+'.'+str(direc)+'.gz',scores)
