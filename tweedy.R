...
summStat=read.table("summStat",stringsAsFactors=F)
metaData=read.table("metaData",stringsAsFactors=F)

fileName=strsplit(args[1],".",fixed=T)[[1]][1]
statRow=as.numeric(metaData[metaData[,1] == fileName,2:4])
caseNo=statRow[3]
ctrlNo=statRow[1]-caseNo
preval=caseNo/(caseNo+ctrlNo)

pval=summStat[,10]
frq=summStat[,6]
zval=abs(qnorm(pval/2))
zval=zval*sign(summStat[,8])
...
summStat[,8]=obj[[1]]
summStatWrite=summStat[summStat[,8] != 0,]
write.table(summStatWrite,"summStat1",row.names=F,col.names=F,quote=F,sep='\t')

summStat[,8]=obj[[2]]
summStatWrite=summStat[summStat[,8] != 0,]
write.table(summStatWrite,"summStat2",row.names=F,col.names=F,quote=F,sep='\t')

summStat[,8]=obj[[3]]
summStatWrite=summStat[summStat[,8] != 0,]
write.table(summStatWrite,"summStat3",row.names=F,col.names=F,quote=F,sep='\t')
